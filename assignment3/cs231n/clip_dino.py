...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    preprocessed_images = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.device = device
        preprocessed_images = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
        self.image_features = clip_model.encode_image(preprocessed_images)
