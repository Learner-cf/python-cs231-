...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        h[:, t, :] = next_h
        cache.append((x[:, t, :], prev_h, prev_c, Wx, Wh, b, next_h, next_c))
        prev_h = next_h
        prev_c = next_c
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    print(f"x shape: {x.shape}, prev_h shape: {prev_h.shape}, prev_c shape: {prev_c.shape}")
    print(f"Wx shape: {Wx.shape}, Wh shape: {Wh.shape}, b shape: {b.shape}")
    N, H = prev_h.shape
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.zeros((N, T, H), dtype=x.dtype, device=x.device)
    prev_h = h0
    prev_c = torch.zeros((N, H), dtype=x.dtype, device=x.device)  # 确保维度正确
    cache = []
