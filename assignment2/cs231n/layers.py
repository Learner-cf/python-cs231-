...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dx = (1. / N) * (1. / np.sqrt(sample_var + eps)) * (N * dx_normalized - np.sum(dx_normalized, axis=0) - x_normalized * np.sum(dx_normalized * x_normalized, axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    out = gamma * x_normalized + beta
    cache = (x.T, x_normalized, sample_mean, sample_var, gamma, beta, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    dx = (1. / D) * (1. / np.sqrt(sample_var + eps)) * (D * dx_normalized - np.sum(dx_normalized, axis=0) - x_normalized.T * np.sum(dx_normalized * x_normalized.T, axis=0))
    dx = dx.T
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    out = gamma * x_normalized + beta
    cache = (x.reshape(N, C, H, W), x_normalized, sample_mean, sample_var, gamma, beta, G, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    x, x_normalized, sample_mean, sample_var, gamma, beta, G, eps = cache
    N, C, H, W = x.shape
    ...
    D = C // G * H * W
    dx = (1. / D) * (1. / np.sqrt(sample_var + eps)) * (D * dx_normalized - np.sum(dx_normalized, axis=1, keepdims=True) - x_normalized * np.sum(dx_normalized * x_normalized, axis=1, keepdims=True))
