    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == 'batchnorm':
                ...
                bn_out, bn_cache = batchnorm_forward(a_out, gamma, beta, self.bn_params[i])
                out, relu_cache = relu_forward(bn_out)
                layer_cache = (a_cache, bn_cache, relu_cache)

            elif self.normalization == 'layernorm':
                gamma = self.params['gamma' + str(idx)]
                beta = self.params['beta' + str(idx)]
                a_out, a_cache = affine_forward(out, W, b)
                ln_out, ln_cache = layernorm_forward(a_out, gamma, beta, self.bn_params[i])
                out, relu_cache = relu_forward(ln_out)
                layer_cache = (a_cache, ln_cache, relu_cache)

            else:
                a_out, a_cache = affine_forward(out, W, b)
                out, relu_cache = relu_forward(a_out)
                layer_cache = (a_cache, relu_cache)

            if self.use_dropout:
                out, dropout_cache = dropout_forward(out, self.dropout_param)
                cache = (layer_cache, dropout_cache)
            else:
                cache = layer_cache

            caches[idx] = cache

        # final affine layer -> scores
        ...
        for i in reversed(range(self.num_layers - 1)):
            ...
            if self.normalization == 'batchnorm':
                a_cache, bn_cache, relu_cache = layer_cache
                drelu = relu_backward(dout, relu_cache)
                dbn, dgamma, dbeta = batchnorm_backward(drelu, bn_cache)
                dout, dW, db = affine_backward(dbn, a_cache)
                grads['gamma' + str(idx)] = dgamma
                grads['beta' + str(idx)] = dbeta

            elif self.normalization == 'layernorm':
                a_cache, ln_cache, relu_cache = layer_cache
                drelu = relu_backward(dout, relu_cache)
                dln, dgamma, dbeta = layernorm_backward(drelu, ln_cache)
                dout, dW, db = affine_backward(dln, a_cache)
                grads['gamma' + str(idx)] = dgamma
                grads['beta' + str(idx)] = dbeta

            else:
                a_cache, relu_cache = layer_cache
                drelu = relu_backward(dout, relu_cache)
                dout, dW, db = affine_backward(drelu, a_cache)
