...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    在训练期间，样本均值和（未校正的）样本方差由小批量统计量计算，并用于对输入数据进行归一化。同时在训练时，
    我们为每个特征维护按指数衰减的运行均值和运行方差，这些运行平均值在测试时用于归一化数据。

    在每个时间步，我们根据动量参数用指数衰减更新均值和方差的运行平均值：

    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    running_var  = momentum * running_var  + (1 - momentum) * sample_var

    注意，批量归一化论文建议的测试时做法有所不同：论文中建议在测试时用大量训练样本直接计算每个特征的样本均值和方差
    ，而不是使用运行平均。本实现选择使用运行平均，因为这样无需额外的估计步骤；`torch7` 的批量归一化实现也使用运行平均。
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dx = (1. / N) * (sample_var + eps) ** (-0.5) * (N * dx_normalized - np.sum(dx_normalized, axis=0)
...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dx = (1. / D) * (sample_var + eps) ** (-0.5) * (D * dx_normalized - np.sum(dx_normalized, axis=1, keepdims=True)
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    w_start = j * stride
                    w_end = w_start + WW
                    out[n, f, i, j] = np.sum(x_padded[n, :, h_start:h_end,
                                             w_start:w_end] * w[f]) + b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    mask = (x_pooling == np.max(x_pooling))
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_grouped = x.reshape(N, G, C // G, H, W)
    D = (C // G) * H * W
    dx_grouped = (1. / D) * (var + eps) ** (-0.5) * (D * dx_normalized_grouped - np.sum(dx_normalized_grouped, axis=(2, 3, 4), keepdims=True)
                                       - (x_grouped - mean) * np.sum(dx_normalized_grouped * (x_grouped - mean), axis=(2, 3, 4), keepdims=True) * (var + eps) ** (-1))
