...
class TwoLayerNet(object):
    ...
    self.params that maps parameter names to numpy arrays.
     ...
     一个两层的全连接神经网络，使用 ReLU 非线性和 softmax 损失，采用模块化层设计。
    假设输入维度为 D，隐藏层维度为 H，并对 C 类进行分类。

    网络结构应为 affine - relu - affine - softmax。

    注意本类不实现梯度下降；相反，它将与一个负责运行优化的独立 `Solver` 对象交互。

    模型的可学习参数存储在字典 `self.params` 中，字典将参数名映射到对应的 numpy 数组。
